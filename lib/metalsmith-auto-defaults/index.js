...
const getFaviconsHtml = (metadata) => {
    ...
    let html = metadata.favicons.html;
    ...
    html = html.filter(e => !faviconsMetaRemove.some(rx => rx.test(e)));

    // see: https://css-tricks.com/svg-favicons-and-all-the-fun-things-we-can-do-with-them/
    // see: https://stackoverflow.com/questions/63878618/why-does-chrome-request-both-svg-and-ico-favicons
    html = html.map(e => e.replace(/rel=\"(shortcut )?icon\"/, "rel=\"alternate icon\""));
...
const plugin = options => {
    ...
    return (files, metalsmith, done) => {
        ...
        Object.keys(files).forEach(file => {
            ...
            // Prepend stylesheet prefix path
            if (data.stylesheet) data.stylesheet = settings.stylesheetPrefixPath + data.stylesheet;
        });
